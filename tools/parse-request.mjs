...
import { selectCatalogGraphContract, validateCatalogGraphContract } from '../site/wrt/lib/catalog-loader.js';
import {
...
async function loadCatalogKconfigSymbols(catalogContract, catalogBranch) {
  let contract;
  try { contract = selectCatalogGraphContract(catalogBranch); }
  catch (error) { fail(error.message); }
  ...
  try {
    ...
    if (contract.jsonBytes !== undefined && json.byteLength !== contract.jsonBytes) {
      throw new Error('Catalog graph JSON byte count mismatch');
    }
    if (contract.sha256 !== undefined && sha256(json) !== contract.sha256) {
      throw new Error('Catalog graph JSON SHA-256 mismatch');
    }
  ...
  try {
    validateCatalogGraphContract(document, contract, {
      commit: catalogContract.sourceCommit, repo: catalogContract.sourceRepository,
    }, { requiredCapabilities: REQUIRED_KCONFIG_RELATION_CAPABILITIES });
  } catch (error) { fail(error.message); }
  let model;
...
// Job-local evidence from the already verified graph. This is not client
