...
- Before compilation, the Worker hashes `.config`, runs `make prepare-tmpinfo V=s`, and requires the before/after hashes to match plus a non-empty real `tmp/.packageinfo`. The closure gate consumes this evaluated native metadata, not a raw Makefile fallback. Source-level build dependencies apply to their binary packages; host dependencies stay separate. Unresolved paths reachable from active concrete roots fail closed; unrelated parse diagnostics do not block every root. A snapshot-bound symbol-kind receipt distinguishes package suboptions from concrete packages, with refreshed native package records taking precedence.
