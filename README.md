...
The Worker checks build compatibility against upstream `.packagedeps`, expanded by GNU Make in a recipe-free adapter. `.packageinfo` supplies concrete package/source identities; there is no second hand-inferred compile graph. Source compilation does not require installing an output package, and disabled multi-provider edges are not mistaken for unresolved dependencies. `BUILD-LOGS` includes `package-info.txt.gz`, `package-deps.mk.gz`, configuration and graph evidence for offline diagnosis. Passing this check is not proof of a successful firmware build.

Large imports reuse snapshot-local dependency indexes and yield between ordered Kconfig intents. A shared loading indicator shows progress while configuration controls are locked; you can still scroll. Required choice dialogs remain usable, failed imports restore the preceding workspace, and controls unlock on completion or error. Import still does not open compatibility recommendations automatically.
...
- Before compilation, the Worker hashes `.config`, runs `make prepare-tmpinfo V=s`, and requires unchanged configuration and non-empty native `.packageinfo` / `.packagedeps`. A recipe-free GNU Make adapter expands the generated compile-edge assignments; it does not include the root build Makefile or run Defconfig. Target/host domains, selected/default variants, self-source filtering and provider guards retain upstream semantics. Missing native graph evidence or active source identities remain inconclusive. The snapshot-bound symbol-kind receipt distinguishes package suboptions from concrete packages, with refreshed native package records taking precedence.
- Mandatory choice defaults follow Catalog's proven native declaration order, including members outside the current Target projection. The shared resolver writes effective choice and typed scalar defaults into both final `.config` and JSON overrides, preserves explicit values, and drops newly inferred scalar defaults when their owner becomes inactive. Missing order or membership evidence stays deferred. Historical imports remain supported; these regressions do not establish parity for every possible upstream configuration.
...
- Recommendations settle inherited preferences, dependent options, and cleanup through the shared Kconfig runtime. Identical executable plans count as one recommendation; genuinely different alternatives remain a user choice. Verify by clicking **Test** again and exporting/reimporting the JSON: restored defaults must not resurrect old imported selections, and a recommendation must not introduce new blocking configuration errors.
