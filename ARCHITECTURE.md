...
E v2 Phase A 只验证 GitHub Actions 的 branch/ref/commit 机制，不接管普通 `[build]` Issue 的生产入口：`build-dispatcher.yml` 与 `build-routing-probe.yml` 都只允许手动 `workflow_dispatch`，且 Dispatcher 只派发 Probe，不调用 `custom-build.yml`。测试 Issue 必须以 `[route-test]` 开头并只携带一个网页生成的 `build-request.json`；Dispatcher 把 GitHub API 读取到的 Issue 固定为本地快照，通过自定义 `REQUEST_EVENT_PATH` 交给 `fetch-build-request.mjs`。正文读取顺序为显式 `ISSUE_BODY` → `REQUEST_EVENT_PATH` → GitHub 原生 `GITHUB_EVENT_PATH`，因此手动探针可读取快照，而正常 Issue 构建仍使用 GitHub 提供的事件文件；不得尝试覆盖保留的 `GITHUB_*` 默认变量。随后 Dispatcher 只读取 `sourceEnv`、完整 40 位 `requestCommit` 与 `requestId`，确认分支当前 HEAD 精确等于请求提交后，才在该 branch/ref 派发只读 Probe Worker。Probe 再核对 `github.ref_name`、`github.sha` 与精确 checkout HEAD。Phase A 不运行 OpenWrt 编译，也不改变 Blog/普通 `[build]` Issue 的生产后端。 / E v2 Phase A validates GitHub Actions branch/ref/commit behavior without taking over normal `[build]` Issue traffic. The manual-only dispatcher snapshots the route-test Issue and passes that snapshot through custom `REQUEST_EVENT_PATH`; `fetch-build-request.mjs` resolves body sources as explicit `ISSUE_BODY`, then `REQUEST_EVENT_PATH`, then GitHub's native `GITHUB_EVENT_PATH`, avoiding any attempt to overwrite reserved `GITHUB_*` defaults. The dispatcher validates branch HEAD against the full request commit and dispatches only the read-only routing probe on that ref; the probe rechecks workflow ref/SHA and checkout HEAD without compiling OpenWrt or invoking `custom-build.yml`.
