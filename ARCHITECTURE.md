...
└─ tools/                        # Node 工具链 / Node tooling
   ...
   ├─ site-release.mjs           # site/wrt 原始字节的确定性全站 SHA-256 / deterministic full-site SHA-256 over raw deployable bytes
...
6. `VERSION` 在本地 Prepare 阶段由 `stamp-site-version.mjs` 按 tools/Workflow/Shell/config/site 项目指纹生成；同时 `site-release.mjs` 对 `site/wrt` 实际发布字节递归计算确定性 `siteSha256`（仅排除自引用 `site-version.json` 与部署实例 `build-meta.json`）。`site-version.json` 保存 Version + project fingerprint + siteSha256；CI 仅以 `--check` 验证。`build-meta.json` 记录 Version/Commit/Branch/Built/siteSha256，必须与 Release Pointer 匹配。Git checkout 字节由 `.gitattributes` 固定，回归夹具同时覆盖 `core.autocrlf=false/true`，两者必须得到同一 raw-byte siteSha256；SHA 算法本身仍不归一化 CRLF/LF。旧八位请求继续兼容 / local Prepare owns VERSION plus a deterministic raw-byte siteSha256; `.gitattributes` stabilizes checkout bytes and the regression matrix proves identical hashes under core.autocrlf false/true without normalizing the hash input; CI validates only, while deployment metadata must match the same release SHA.
