...
Build environment identity is deployment metadata, not application configuration. `build-meta.branch` is generated from the deployment branch; `site/wrt/lib/build-identity.js` is the single naming authority shared by the browser and request parser. Every non-`main` branch adds the same sanitized branch identity to `[build]` Action titles and Artifact names; `main` remains unprefixed. The page loads `site-version.json` and `build-meta.json` together with `cache: no-store`; cloud submission stays disabled unless that fresh pair yields one valid version/branch/full-commit identity. No hostname or provider-specific branch detection belongs in `app.js`.
...
E v2 Phase A 已用真实 GitHub Actions Run 验证 main Dispatcher → exact branch/ref → Probe Worker：dev 正向、含 `/` 分支、stale commit 拒绝、branch 不存在拒绝均通过。Phase B1 又以 Issue #138 / Run 31280106097 验证真实 `custom-build.yml` Worker 的 `REQUEST_BRANCH=WORKFLOW_BRANCH=dev`、`REQUEST_COMMIT=WORKFLOW_COMMIT=63aafb274720345df1d5d659dbdebb2307865dd7`，且该真实 OpenWrt 构建最终全绿，因此 exact-ref Worker 路由与完整 Build Backend 均已通过。Phase B2 将普通 `[build]` Issue 的唯一生产入口切到默认分支 `build-dispatcher.yml`：Dispatcher 固定 opened 事件快照，只从 schema 5 `build-request.json` 读取 `sourceEnv`、完整 `requestCommit` 与 `requestId`，要求远端 branch HEAD 精确等于请求提交，并在派发前再次核对 Issue state/author/created_at/title/body。通过后用 `workflow_dispatch(ref=sourceEnv)` 启动该分支上的 `custom-build.yml`；Worker 不再监听 `issues: opened`，并再次核对 workflow branch/SHA、Issue 快照、checkout HEAD 与 JSON identity。`probe` 与 Owner-only `build-canary` 仍保留为手动诊断模式。`cancel-build.yml` 按原 Issue 作者 + Issue 编号识别新的 workflow_dispatch Worker，并暂时兼容切流前仍在运行的 direct-Issue Worker。网页的 `site-version.json` + `build-meta.json` 改为同轮 `no-store` fresh identity；缺失或版本/branch/full-SHA 不一致时只允许页面继续使用，不允许提交云编译。全站 SHA-256 内容缓存仍属于后续独立 F。 / Phase A passed real exact-ref Probe routing, and Phase B1 then proved a real `custom-build.yml` Worker runs on the requested dev ref and full commit. Phase B2 makes the default-branch Dispatcher the single production `[build]` Issue entrypoint. It freezes and revalidates the Issue, reads only the schema-5 route envelope, requires branch HEAD to equal the full request commit, and dispatches `custom-build.yml` on that exact ref. The Worker is workflow_dispatch-only and rechecks workflow ref/SHA, frozen Issue identity/body, checkout HEAD, and JSON identity. Manual Probe and owner-only build-canary remain available. Cancellation matches the original Issue author/number and keeps temporary compatibility with pre-cutover direct-Issue runs. The page now obtains site-version and build-meta as one fresh no-store deployment identity; cloud submit is disabled unless a valid matching version/branch/full-SHA pair exists. B2.1 also treats Workflow `run-name` as a YAML semantic boundary: any plain-scalar `#` hazard is rejected by the generic gate, while quoted `#` expressions are allowed. Full-site SHA-256 caching remains a separate F task.
