...
│  ├─ build-dispatcher.yml       # E v2 Phase B1 手动 exact-ref Probe/真实 Worker canary 派发器 / manual exact-ref Probe/real-Worker canary dispatcher
...
E v2 Phase A 已用真实 GitHub Actions Run 验证 main Dispatcher → exact branch/ref → Probe Worker：dev 正向、含 `/` 分支、stale commit 拒绝、branch 不存在拒绝均通过。Phase B1 仍不接管普通 `[build]` Issue 的生产入口，而是在同一个 manual-only `build-dispatcher.yml` 中增加显式 `mode=build-canary`，仅仓库 Owner 可用；默认 `mode=probe` 保留 Phase A 的只读探针。Dispatcher 继续通过 GitHub API 固定 Issue 快照，经自定义 `REQUEST_EVENT_PATH` 交给 `fetch-build-request.mjs`，并只从 schema 5 `build-request.json` 读取 `sourceEnv`、完整 40 位 `requestCommit` 与 `requestId`。两种模式都要求远端 branch HEAD 精确等于请求提交，并在派发前再次核对 Issue 的 state/author/created_at/title/body 未变化。`probe` 只派发 `build-routing-probe.yml`；`build-canary` 则把同一 Issue 快照与 exact branch/commit 作为既有 `custom-build.yml` 的 workflow_dispatch inputs，在请求 ref 上做一次真实 Build Worker canary。为避免 B1 触发当前 `custom-build.yml` 的 `issues: opened` 生产入口，真实 canary Issue 必须先以 `[route-test]` 创建并上传完整网页请求，再由维护者把标题编辑成与请求身份匹配的 `[build] ...` 后手动运行 `build-canary`；标题编辑不是 `opened` 事件。B1 不修改 `custom-build.yml`，普通用户与 Blog 构建链保持原状，只有真实 Worker canary 通过后才允许设计 Phase B2 生产切流。正文读取顺序仍为显式 `ISSUE_BODY` → `REQUEST_EVENT_PATH` → GitHub 原生 `GITHUB_EVENT_PATH`，不得覆盖保留的 `GITHUB_*` 默认变量。 / E v2 Phase A has passed real GitHub Actions routing runs for dev, slash-containing branches, stale-commit rejection, and missing-branch rejection. Phase B1 still leaves normal `[build]` production traffic untouched. The manual-only dispatcher keeps `probe` as the default and adds an owner-only `build-canary` mode. Both modes snapshot and revalidate the Issue, read the schema-5 routing envelope through `REQUEST_EVENT_PATH`, require remote branch HEAD to equal the full request commit, and dispatch on that exact ref. Probe mode remains read-only; build-canary dispatches the existing `custom-build.yml` Worker with the frozen Issue identity/body and exact request branch/commit. A real canary Issue is created first as `[route-test]`, then its title is edited to the matching `[build] ...` identity before the owner manually dispatches build-canary, preventing the current `issues: opened` production entry from auto-starting the canary. B1 does not modify the production Worker or normal Blog/user flow; Phase B2 remains blocked until the real Worker canary passes.
