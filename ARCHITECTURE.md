...
Local Preview 不使用工作区内可能过期的 `data/build-meta.json`；本地静态服务器复用正式生成器，在内存中投影当前 VERSION、siteSha256、Git branch 与完整 HEAD。该投影不写回工作区，只负责让本地页面采用与代码分支一致的 Catalog 通道和 exact-ref 请求身份。未提交文件只存在于浏览器预览，云构建仍指向显示的已提交 HEAD。

Local Preview does not trust a possibly stale tracked `data/build-meta.json`. The static server reuses the production generator and projects the current VERSION, siteSha256, Git branch, and full HEAD in memory without writing the workspace. This keeps the local Catalog channel and exact-ref request identity aligned with the code branch; uncommitted files remain preview-only, while cloud builds still target the displayed committed HEAD.
...
规则只保存 Kconfig/Catalog 无法表达的证据事实和稳定 ID。读取端和发布端只接受 schema 2：通用 `issue` 与 `match` 区分文件归属和已知构建失败，允许单包或多包；schema 1（含 `ownership`）及其他 schema 一律拒绝。fix、dev、staging、main 分别读取同名 Catalog 数据通道，必须先在同名候选通道验证契约再晋级。浏览器通过现有 `model.byPackage` 取得 config symbol、类型、N/M/Y、依赖与反向依赖，并由共享引擎同时派生 Advanced、即时冲突框和兼容性提示的当前可选状态；bool 直接依赖的 `m` 按原生 Kconfig 提升为 `y`。`all-installed` 只在全部为 `y` 时触发，`all-selected` 对全部 `m/y` 触发。候选方案继续通过同一个 `applyUserIntent` 推导，只有唯一最低变更成本方案才可自动推荐；应用成功后窗口保留并刷新状态，用户明确关闭后才继续。软规则只在自检与请求创建时运行。强制确认绑定规则 SHA + 数据通道 + Source/Branch + 配置 revision；后端仅保存强制规则 ID 审计，不复判、不上锁、不改配置。

The root `assets.compatibility` contract points to one small global `compatibility.json.gz` pinned by compressed SHA. AutoBuild code channels read only their matching data channels, and preview channels never fall back to the production Release. Publishers and readers accept schema 2 only: its generic issue/match semantics cover one or many packages, while schema 1 (including `ownership`) and every other schema are rejected. Fix, dev, staging, and main validate the contract in their matching candidate data channel before promotion. The browser resolves package IDs through Catalog; `all-installed` requires all `y`, while `all-selected` accepts all `m/y`. Plans use the single existing `applyUserIntent` executor without duplicating symbol/state/dependency data, and an applied recommendation remains visible until the user closes the dialog. Soft rules run only at self-test and request creation. Force acknowledgement is revision-bound, while the backend records forced IDs without re-evaluation, package locks, source patches, or configuration changes.
