...
6. `site-version.yml` 在 `tools/**`、`.github/workflows/**`、`Shell/**`、`config/**`、`site/wrt/**` 或 `VERSION` push 后按统一输入指纹同步根 `VERSION` 与静态 `site-version.json`；旧八位请求继续兼容，actor 条件阻断机器人提交循环 / site-version automation fingerprints tools, workflows, Shell, config, and site inputs, keeps root `VERSION` and static `site-version.json` together, accepts legacy eight-digit requests, and prevents bot-commit loops
