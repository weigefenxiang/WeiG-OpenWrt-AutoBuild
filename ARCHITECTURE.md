...
5. 下载失败记录警告后继续,编译失败直接结束；四类 Artifact 均以请求编号开头，如 `006_01-FIRMWARE-ALL-…`，日志保留 14 天并回评 @提交者 / download errors warn and continue, while compile errors fail directly; all four artifacts begin with the request reference, for example `006_01-FIRMWARE-ALL-…`, with logs retained for 14 days and the submitter mentioned in the Issue
