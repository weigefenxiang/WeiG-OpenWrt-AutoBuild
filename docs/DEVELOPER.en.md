...
Probe submission uses schema 2: it runs the same final config-generation path as Advanced menuconfig, extracts every resolved `CONFIG_PACKAGE_*=m/y` entry, and preserves each exact M/Y state; Source/Branch and Target coverage remain probe controls. Catalog consumes this package Kconfig state directly and no longer maps curated application IDs back to packages through `applications.json.gz`.
