...
- Downloads and compilation both use `JOBS=$(( $(nproc) + 1 ))` dynamic concurrency. `stdbuf` + `tee` streams raw output line by line in Actions while preserving complete `download.log` / `build.log` files. Download errors warn and continue; a compile error fails the job directly, with no low-noise filter or single-thread retry.
...
**Reading failures**: instant red = parameter validation, so read the `parse-request` step output. Download and compile steps stream raw output live while also writing complete files; if GitHub truncates a long console log, use the `BUILD-LOGS-…` artifact. For compile-stage red, read in order: `00-SUMMARY.txt` → `build-metadata.txt` → full `download.log` / `build.log` → `errors.txt` / `tail-200.txt` → `package-logs.tar.gz` → `final.config`. Common causes: `Package xxx is missing`, out-of-space, or a temporary upstream breakage.
