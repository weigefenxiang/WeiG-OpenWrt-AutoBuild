...
- Downloads and compilation both use `JOBS=$(( $(nproc) + 1 ))` dynamic concurrency. `stdbuf` + `tee` streams raw output line by line in Actions while preserving complete `download.log` / `build.log` files. After `make defconfig`, CI reads the selected upstream's `tmp/.packageinfo` and rejects enabled pairs declared by `Conflicts:`; the page and Issue parser use the same Catalog metadata for early feedback. `CONFIG_DEVEL=y` and `CONFIG_BUILD_LOG=y` keep per-package logs available. A parallel compile failure gets one `make -j1 V=s` retry capped at 60 minutes: a successful retry is marked as recovered and continues to artifacts; a failed or timed-out retry remains failed with `build-diagnostic.log` uploaded.
