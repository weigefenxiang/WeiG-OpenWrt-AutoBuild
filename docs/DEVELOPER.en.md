...
The bottom-right web **检** control opens the existing self-test immediately. Its header shows **Package compatibility probe** before Close; clicking it opens a responsive in-page workspace. The Probe reuses the current Source/Branch Advanced menuconfig/Kconfig search text, matching, and ranking instead of maintaining a second package search model; Source/Branch inventory still comes from Catalog, while generic probe UI text is AutoBuild-local and does not require `applications.json.gz`. Both views rank `luci-app-*` packages first, an exact ordinary package second, other `PACKAGE_*` matches next, and non-package upstream Kconfig options last. Advanced and Probe also share one package-selection relation: when current Kconfig exposes both `PACKAGE_<name>` and `PACKAGE_luci-app-<name>`, selecting either package row resolves to the LuCI application as the canonical root and then uses the same `applyMenuValue()` path to preview its mandatory dependency closure. Ordinary packages without a matching LuCI application keep their own identity. the shared Advanced menuconfig package state contains only canonical root `PACKAGE_*` choices; each tested Source/Branch/Target expands dependencies again through its own Kconfig, and ordinary Kconfig rows remain reference-only. Desktop keeps one `L1–L4` row; phones use a compact 2×2 grid with short depth labels and hide package title/usage behind a single-tap `!` detail affordance; help is revealed only by hover, keyboard focus, or clicking ⓘ. Source/Branch scope and Target coverage share one select row, while custom scope exposes searchable checkboxes. The bounded-height desktop modal gives its only normal scrollbar to results; only low-height screens enable emergency outer scrolling. Results preserve complete IDs, reuse the global menu tooltip for truncated text, and accept `CONFIG_PACKAGE_*`, `PACKAGE_*`, package-name, and short-ID search forms through the shared search path. The plan starts collapsed, and Preview and Submit remain the final actions. Submit downloads exactly one the shared Advanced menuconfig package state and opens the dedicated Catalog Issue form.

The browser schema-1 request sends one to eight short package names derived only from selected `PACKAGE_*` symbols; all, current, or explicit Source/Branch entries; and automatic, current Target/Profile, or all representative Target coverage. The controller reads and verifies `index.json`, `applications.json.gz`, and each matched Branch's `core` shard from the data branch paired with the code channel. It maps application IDs to real packages before creating the dynamic Matrix and keeps no Source/Branch or Target version list.
...
The browser downloads a public, auditable the shared Advanced menuconfig package state, and the user drags it into the dedicated Catalog Issue form's only required upload field. A lightweight gateway on the default branch validates the single attachment, UTF-8/JSON/schema, SHA-256, permission, and Issue identity, then dispatches the worker to the exact code channel in the request. The worker downloads and hashes the same attachment again before validating asset contracts, package mappings, Source/Branch, and Target/Profile and creating a Matrix. The gateway is Issue-event driven and has no polling. It must first reach the default branch; afterward the browser can validate the complete dev/staging/main routes. `workflow_dispatch` remains the administrator fallback. The requester or a write/maintain/admin collaborator can reply with exactly `/cancel` in the same Issue; normal cancellation runs before force cancellation.
