...
Immediately before compilation, the Worker hashes the authoritative `.config`, runs `make prepare-tmpinfo V=s`, and requires the before/after hashes to match and a non-empty real `tmp/.packageinfo`. The build-closure gate consumes the evaluated native stream without a raw Makefile fallback. It respects Description/Config blocks and source-level fields (including `Build-Depends/host`); source build dependencies apply to every binary package in that source, while host dependencies remain separate. Unknown paths reachable from an active root fail closed; unrelated parse diagnostics do not poison the entire selection.
