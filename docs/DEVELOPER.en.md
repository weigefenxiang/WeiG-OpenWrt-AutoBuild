...
- Downloads and compilation both use `JOBS=$(( $(nproc) + 1 ))` dynamic concurrency. `stdbuf` + `tee` streams raw output line by line in Actions while preserving complete `download.log` / `build.log` files. After `make defconfig`, CI reads the selected upstream's `tmp/.packageinfo` and rejects enabled pairs declared by `Conflicts:`; the page and Issue parser use the same Catalog metadata for early feedback. `config/001.presets/config-rules.json` (public copy under `site/wrt/data/`) is the single source for semantic-choice and known upstream-compatibility rules. Its schema supports Source/Branch/Target scopes, `when.all`, `when.any`, scalar or Y/M state arrays, `set`, and `setPrefixes`; `prompt: always` requires an explicit choice for both new and imported configurations, while `maintenance` records the observed versions, evidence run, and review condition. The page resolves rules and prepares the request before opening GitHub, without pre-opening `about:blank`; direct legacy attachments remain explicitly rejected and are never silently altered. A rule now covers the external ksmbd 3.5.4 VFS API incompatibility on LEDE master with Linux 6.12 and disables its kernel module, server, LuCI app, and translations together. A preflight block explicitly says compilation did not start. `CONFIG_DEVEL=y` and `CONFIG_BUILD_LOG=y` keep per-package logs available. A parallel compile failure gets one `make -j1 V=s` retry capped at 60 minutes: a successful retry is marked as recovered and continues to artifacts; a failed or timed-out retry remains failed with `build-diagnostic.log` uploaded.
