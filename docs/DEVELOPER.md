...
推荐规划与实际应用共用同一 Kconfig 求值器及用户偏好、保护项、依赖和显式意图上下文。直接操作、偏好恢复、孤儿清理和 choice 切换后都要传播依赖变化；tristate 上限降到 M 时不应直接降成 N。实际步骤和最终变化都相同的候选合并为一个推荐，真正不同的等价方案仍由用户选择。已关闭父项留下的失效子项可通过通用重新求值推荐恢复，不重新启用父项，不维护包名特例。

应用推荐不能引入新的阻断问题，失败时恢复编辑快照。将导入值恢复为原生默认值后，即使用户覆盖记录被清除，导出也必须使用当前有效值，不能复活旧导入文本；只额外覆盖发生变化的已知符号，未知内容仍按既有策略保留。验收应包括再次点击“检”、导出 JSON、重新导入后的配置值一致性，而非只看首次检查。浏览器验证不等于固件编译通过。
