...
网页右下角“检”立即打开原自检界面；自检标题栏在关闭按钮左侧显示“插件兼容探针”，点击后打开响应式网页内工作区。Probe 直接复用当前 Source/Branch 的 Advanced menuconfig/Kconfig 搜索文本、匹配和排序，不再维护第二套 package 搜索模型；Source/Branch 清单仍来自 Catalog，通用 Probe UI 文案属于 AutoBuild 且不依赖 `applications.json.gz`。Advanced 与 Probe 统一排序为：`luci-app-*` 软件包优先、与关键词精确对应的普通软件包其次、其它 `PACKAGE_*` 软件包随后、非软件包的上游 Kconfig 选项最后。两者也共用同一 package 选择关系：当前 Kconfig 同时存在 `PACKAGE_<name>` 与 `PACKAGE_luci-app-<name>` 时，用户选择统一归到 LuCI application，并由 Kconfig 处理依赖闭包；无对应 LuCI application 的普通 package 保持自身身份。Probe 中 package 行按该 canonical 关系选择，普通 Kconfig 行只作参考展示。四个深度以 `L1–L4` 单行显示，说明只在悬浮、键盘聚焦或点击 ⓘ 后出现；Source/Branch 范围与 Target 覆盖使用同一行下拉框，自定义范围提供可搜索多选。弹窗采用受限高度，正常桌面只让结果区滚动；低高度窗口才启用外层应急滚动。结果保留完整 ID，截断内容复用全站菜单浮层；`CONFIG_PACKAGE_*`、`PACKAGE_*`、package name 和短 ID 全部走同一搜索链。计划默认折叠，预览与提交固定为内容末尾操作；提交只把真实短 package name 写入唯一的 `probe-request.json`，并打开 Catalog 专用 Issue 表单。
