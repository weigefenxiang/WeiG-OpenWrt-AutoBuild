...
│  ├─ build-dispatcher.yml         ★默认 main 的 Issue 路由器:只核对 request branch+commit 并派发 Worker
│  ├─ custom-build.yml             ★核心 Worker:workflow_dispatch 精确 branch+commit;独立固件 + 四类辅助产物
...
- 构建入口拆成稳定 Dispatcher 与 branch-aware Worker：`.github/workflows/build-dispatcher.yml` 是默认 `main` 上唯一的 `issues: opened` 入口，只解析网页预填的 `WEIG_BUILD_ROUTE_V1`（branch + 完整 40 位 commit）、核对 branch 仍存在且 HEAD 与请求 commit 完全一致，然后 `workflow_dispatch(ref=request_branch)` 到 `.github/workflows/custom-build.yml`。Worker 启动时再次要求 `GITHUB_REF_NAME/GITHUB_SHA` 与请求一致，`actions/checkout` 固定到同一 `request_commit`，`parse-request.mjs` 再核对 JSON 的 `sourceEnv/requestCommit` 与 Worker 输入一致；任何旧网页请求、branch 已前进、branch 不存在或身份错配都直接阻止，不会悄悄使用新 HEAD。Issue 创建时正文作为快照传入 Worker，避免后续编辑改变真正构建输入；`cancel-build.yml` 先关闭 Issue，再取消同一 Issue 的 Dispatcher/Worker，以关闭取消与派发竞态。`repository_dispatch`、旧 smoke 配置生成入口仍不存在。
- 部署实例的 `build-meta.branch`/`commit` 同时承担网页构建来源身份：`main` 保持原来的无前缀 Issue/Actions/Artifact；任何非 `main` 请求都使用实际 branch 作为前缀，branch 内部 `/` 统一替换为 `_`，例如 `fix/foo` → `[build] fix_foo/请求时间戳/...` 与 `fix_foo-...` Artifact。该通用规则与隐藏路由标记都实现在 `site/wrt/lib/build-identity.js`，不按域名判断，也不为 dev/staging/fix/feat 写特判。`build_ref` 仍为稳定的 `请求时间戳-构建标识`，`artifact_ref` 只负责环境感知命名；请求 branch/commit 与实际 Workflow branch/commit 继续写入 Summary/`build-metadata.txt`。云编译提交要求有效 branch + 完整 commit；Local Preview 启动前用 `prepare-web-deployment.mjs` 生成 gitignored `build-meta.json`，无 Git 的静态副本仍可浏览但不会伪造可提交身份。时区、主题、NTP、请求/生效软件包镜像与 APK/OPKG 检测结果在提交配置、Summary、`package-mirror-report.json`、`firmware-settings.txt` 与固件内 `/etc/weig-build-info` 交叉核验；固件/config 保留 30 天，完整日志保留 14 天。
...
- Issue 仍是用户唯一构建入口，但执行链是：网页完整 config + exact route → Issue → main Dispatcher → 对应 branch/commit Worker → `submitted.config` → `openwrt/.config`。旧 `build-config.mjs`、`repository_dispatch` 和 smoke 兼容入口已删除。
...
- 页面整个 `site/wrt/` 目录拷走即可用；Catalog 使用精确缓存 → GitHub Raw 最新 index → jsDelivr/GitHub Raw 固定提交分片 → 完整 GitHub Release，VPS 不保存 Catalog；其他静态数据使用相对路径和本地优先降级。`OpenWebPage_打开网页.bat` 在本地预览运行期间可输入 `0` 正常停止 `wrt-server` 并返回启动器主菜单；直接以 `local` 参数调用时则停止后退出。
