...
网页右下角“检”立即打开原自检界面；自检标题栏在关闭按钮左侧显示“插件兼容探针”，点击后打开响应式网页内工作区。Probe 直接复用当前 Source/Branch 的 Advanced menuconfig/Kconfig 搜索文本、匹配和排序，不再维护第二套 package 搜索模型；Source/Branch 清单仍来自 Catalog，通用 Probe UI 文案属于 AutoBuild 且不依赖 `applications.json.gz`。Advanced 与 Probe 统一排序为：`luci-app-*` 软件包优先、与关键词精确对应的普通软件包其次、其它 `PACKAGE_*` 软件包随后、非软件包的上游 Kconfig 选项最后。选择语义完全服从当前 Kconfig：选择上层 package 时可由依赖闭包自动带上其必需依赖，但选择依赖 package 不得反向选择任何依赖它的上层 package。Probe 中每个 `PACKAGE_*` 行保持自身作为用户明确选择的根 package，依赖展开交给实际探测 Source/Branch/Target 的 Kconfig，普通 Kconfig 行只作参考展示。四个深度以 `L1–L4` 单行显示，说明只在悬浮、键盘聚焦或点击 ⓘ 后出现；Source/Branch 范围与 Target 覆盖使用同一行下拉框，自定义范围提供可搜索多选。弹窗采用受限高度，正常桌面只让结果区滚动；低高度窗口才启用外层应急滚动。结果保留完整 ID，截断内容复用全站菜单浮层；`CONFIG_PACKAGE_*`、`PACKAGE_*`、package name 和短 ID 全部走同一搜索链。计划默认折叠，预览与提交固定为内容末尾操作；提交只把真实短 package name 写入唯一的 `probe-request.json`，并打开 Catalog 专用 Issue 表单。
