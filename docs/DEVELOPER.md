...
- Target 搜索框与“当前构建契约”共用外层卡片和 42px 顶栏高度；展开内容跨越两列，以两组键值并排显示。插件操作不得把全配置 violations 输出到 Toast；直接 package/choice 冲突使用主题对话框展示相关项 N/M/Y。推荐项配置按钮始终保留位置，推荐项关闭时禁用而不是隐藏。
