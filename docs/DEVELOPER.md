...
- schema 2 用 `issue=file-ownership|build-failure` 与 `match=all-installed|all-selected` 组合通用语义，`packages` 接受 1–16 个真实 package ID，`if` 可选，`paths` 只属于文件归属证据。读取端兼容旧 schema 1，内部统一规范化为 schema 2；插件名称、symbol 能力和依赖不复制进规则。
